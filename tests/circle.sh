...
sudo lxc-attach -n "$(docker inspect --format "{{.Id}}" jobmanager)" -- bash -c "/opt/flink/bin/flink run -c opentraffic.accumulator.Accumulator /job/$(basename ${flink_job}) --file valhalla_data/*.csv --reporter http://reporter:${reporter_port}/report?"
